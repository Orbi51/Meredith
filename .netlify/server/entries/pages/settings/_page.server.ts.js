...
import { d as getSettings, r as replaceWorkingHours, m as updateSettings, o as getWorkingHours, h as getCalibrationSamples } from "../../../chunks/queries.js";
import { b as buildCalibrationTable, A as ALL_KINDS } from "../../../chunks/index3.js";
import { r as replan } from "../../../chunks/planner.js";
import { e as ensureRecurringAdmin, u as upcomingAdmin } from "../../../chunks/freelance.js";
import { p as pushConfigured } from "../../../chunks/notify.js";
...
const load = async (event) => {
  ...
  return {
    ...
    upcomingAdmin: upcomingAdmin(/* @__PURE__ */ new Date(), timezone).map((item) => ({
      title: item.title,
      deadline: item.deadline,
      estimateHours: item.estimateHours
    })),
    calibration: ALL_KINDS.map((kind) => ({
      kind,
...
const actions = {
    ...
    for (const dayOfWeek of [0, 1, 2, 3, 4, 5, 6]) {
      ...
      for (const slot of [0, 1]) {
        ...
        intervals.push({
          ...
          preferredKind: preferred === "creative" || preferred === "admin" ? preferred : null
        });
      }
  ...
  generateAdmin: async (event) => {
    ...
    return {
      ok: true,
      message: created === 0 ? "Nothing to add — the upcoming admin is already on the list." : `${created} recurring admin task${created === 1 ? "" : "s"} added.`
    };
  }
};
export {
