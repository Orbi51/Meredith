...
{#if !data.signedIn}
	...
	<p class="mt-2 max-w-lg text-sm text-neutral-600">
...
{:else}
	<div class="flex items-baseline justify-between">
		...
		<span class="text-sm text-neutral-500">{day(data.now)}</span>
	</div>
	...
	{#if data.atRisk.length > 0}
		<section class="mt-4 rounded bg-red-50 p-3">
			<h2 class="text-sm font-medium text-red-800">Deadline passed</h2>
	...
	{:else}
		<ul class="mt-6 space-y-2">
			{#each data.blocks as block (block.id)}
				<li
					class="rounded border p-3 {block.id === data.nextBlockId
						? 'border-neutral-900'
						: 'border-neutral-200'} {block.status === 'skipped' ? 'opacity-50' : ''}"
				>
					<div class="flex items-baseline gap-3">
						...
						<span class="text-sm">
							{#if block.projectName}
								<span class="text-neutral-500">[{block.projectName}]</span>
							{/if}
							{block.title}
						</span>
						{#if block.pool === 'machine'}
							<span class="rounded bg-neutral-100 px-1.5 py-0.5 text-xs text-neutral-500">
								unattended
							</span>
						{/if}
