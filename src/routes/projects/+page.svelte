...
{#if form?.message}
	<p class="mt-3 rounded bg-neutral-100 p-2 text-sm text-neutral-700">{form.message}</p>
{/if}
			...
			<tr class="border-t border-neutral-200 align-top">
				<td class="py-2">
					...
					{#if project.clientName}
						<span class="text-xs text-neutral-400">{project.clientName}</span>
					{/if}
					{#if project.deadline}
						<span class="block text-xs text-neutral-500">due {day(project.deadline)}</span>
					{/if}
				</td>
				...
				<td class="py-2">
					{#if project.feeFormatted}
						...
						{#if project.currency !== 'EUR'}
							...
							{:else}
								<span class="block text-xs text-amber-700">no rate yet</span>
							{/if}
						{/if}
					{:else}
						—
				...
				<td class="py-2">
					...
					{#if project.plannedHours > 0}
						<span class="block text-xs text-neutral-400">+{project.plannedHours}h planned</span>
					{/if}
				...
				<td class="py-2 text-xs text-neutral-500">{project.status}</td>
				...
				<tr class="border-t border-neutral-100 bg-neutral-50">
					<td colspan="7" class="p-3">
						<form method="POST" action="?/update" class="flex flex-wrap items-end gap-3">
							...
							<label class="block">
								<span class="text-xs text-neutral-500">Currency</span>
								<select
									name="currency"
									class="mt-1 rounded border border-neutral-300 px-2 py-1 text-sm"
								>
							...
							<label class="block">
								<span class="text-xs text-neutral-500">Agreed hours</span>
								<input
									name="agreedHours"
									type="number"
									step="0.5"
							...
							<label class="block">
								<span class="text-xs text-neutral-500">Colour</span>
								<input
									name="color"
									type="color"
									value={project.color}
									class="mt-1 h-8 w-14 rounded border border-neutral-300"
								/>
							</label>
							<label class="block">
							...
							<label class="block">
								<span class="text-xs text-neutral-500">Rate to EUR (optional)</span>
								<input
									name="fxRateToEur"
									type="number"
									step="0.000001"
						...
						{#if project.currency !== 'EUR'}
							<form method="POST" action="?/refreshRate" class="mt-3 flex flex-wrap items-end gap-2">
								<input type="hidden" name="projectId" value={project.projectId} />
								<label class="block">
									<span class="text-xs text-neutral-500">Rate on date</span>
									<input
										name="onDate"
										type="date"
										max={today}
								...
								<button class="rounded border border-neutral-300 px-3 py-1.5 text-sm">
									Fetch ECB rate
								</button>
...
<p class="mt-6 max-w-2xl text-xs text-neutral-500">
