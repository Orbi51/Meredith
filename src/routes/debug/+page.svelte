		...
		<tbody>
			{#each data.output.blocks as block (block.taskId + block.start.toISOString())}
				<tr class="border-t border-neutral-200">
					...
					<td class="text-neutral-500">{block.pool}</td>
...
<section class="mt-6">
	...
	{#if data.output.unplaced.length === 0 && data.output.atRisk.length === 0}
		<p class="mt-2 text-sm text-neutral-600">Everything fits.</p>
	{/if}
...
<details class="mt-6">
	<summary class="cursor-pointer text-sm text-neutral-500">Raw output</summary>
	<pre class="mt-2 overflow-x-auto rounded bg-neutral-900 p-3 text-xs text-neutral-100">{JSON.stringify(
			data.output,
			null,
			2
		)}</pre>
